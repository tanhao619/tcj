...
public final class ExcelExport {
     ...
     *        sheet名称和表头值
     */
    public static <T> HSSFWorkbook excelExport(List<?> dataList, Map<String, String> titleMap, String sheetName ,HttpServletResponse response) {
        // 初始化workbook
...
//                    headstyle.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 居中
                    ...
                    sheet.autoSizeColumn(j,true); //设置表格宽度自适应
//                    sheet.setColumnWidth(j, value.getBytes().length*2*255);
                    j++;
                }
                i++;
            }
