...
ProConvention.initColumn = function () {
    return [
        ...
        {title: '', field: 'name', visible: true, align: 'center', valign: 'middle', formatter: ProConvention.formatOperate,width:'10%'},
...
ProConvention.formatOperate = function (val, row, index) {
    ...
    var id = row.id;
    var btn_pass = "<div class='delete_wrap'><button class='btn opearteLog'   onclick='ProConvention.audOne(" + id + ")' style='color:#7f8fa4;padding-left: 3px;'>通过</button></div>";
 ...
 * 审核通过一行
 */
ProConvention.audOne = function(row12) {
    var operation = function () {
        var ajax1 = new $axRest(Feng.ctxPath + "/proConvention/audit/"+row12, "post", function (data) {
            Feng.success("成功！");
            ProConvention.table.refresh();
            setTimeout(function(){
                $(".conventionLog table tr").hover(function(){
    ...
    Feng.confirm("是否审核通过该条履约记录？", operation);
};
...
$(function () {
    ...
    ProConvention.deptLevel = parent.document.getElementById("deptLevel").value;
