...
ExportConfig.sureExport = function () {
    ...
    var url = "/bigProject/exportBigExcel?name="+name+"&StartTime="+StartTime+
        "&EndTime="+EndTime+"&workProcessTime="+workProcessTime;
    if (name.length < 1) {
        Feng.alert ("请勾选需要导出的内容!");
    }else {
        $("#excelBigExp").attr("href",url);
    }
    // Feng.confirm("确认导出勾选?");
};
$(function () {
...
// });
//时间段
$("#inpstart").focus(function(){
    timeBetween($("#inpstart"),$("#inpend"));
});
