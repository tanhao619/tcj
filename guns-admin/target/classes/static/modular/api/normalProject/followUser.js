    ...
    var ajax2 = new $axRest(Feng.ctxPath + "/normalProject/followsPro", "get", function (data) {
        ...
        data.forEach(function (value, index, array) {
            ...
            if (index>=start && index<start+FollowUser.pageSize){
                var node = "<li><label><input type='radio' name='office' value='" + value.id + "'><span>" + value.name + "</span></label><input type='hidden' name='phone' value='"+value.phone+"'></li>";
...
FollowUser.goPage = function (page,this1) {
    ...
    FollowUser.user.forEach(function (value, index, array) {
        ...
        if (index>=start && index<(start+FollowUser.pageSize)){
            var node = "";
            $(".personInfo").append("<li><label><input type='radio' name='office' value='" + value.id + "'><span>" + value.name + "</span></label><input type='hidden' name='phone' value='"+value.phone+"'></li>");
        }
    });
};
