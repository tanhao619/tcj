...
FollowOfficeAndPlat.submit = function () {
    if(this.validate()) {
        Feng.error("跟踪科室必须选择1个及以上（不含信息中心）！");
        return;
    }
    this.clearData();
    this.collectData();
...
$(function () {
    // 加载跟踪投促局科室多选框（信息中心默认不可点）
    var ajax1 = new $axRest(Feng.ctxPath + "/normalProject/followsOffice", "get", function (data) {
        data.forEach(function (value, index, array) {
            var node = "<li><labe1><input type='checkbox' name='office' value='"+value.id+"'>"+value.fullname+"</labe1><input type='hidden' name='id' value=''></li>";
            if (value.fullname === "信息中心") {
                node = "<li class='li-disable'><labe1><input type='checkbox' name='office' value='"+value.id+"'>"+value.fullname+"</labe1><input type='hidden' name='id' value=''></li>";
            }
    ...
    var ajax2 = new $axRest(Feng.ctxPath + "/normalProject/followsPlat", "get", function (data) {
        data.forEach(function (value, index, array) {
            var node = "<li><label><input type='checkbox' name='office' value='" + value.id + "'>" + value.fullname + "</label><input type='hidden' name='id' value=''></li>";
